...
Дополнительно бот умеет хранить несколько резюме в рамках чата, не допускает одинаковые названия, позволяет обновлять резюме, выбирать активное резюме для вакансии и генерировать вопросы по позиции в целом с учетом грейда.
...
- библиотеку нескольких резюме, защиту от дублей названий и обновление резюме;
...
Бесплатный пробный режим по вакансиям ограничен 3 разборами на пользователя. Для каждой вакансии бот дает 5 вопросов, оценивает ответы по ним и дальше показывает заглушку пакета дополнительных тренировок. Режим `/position` отдельно дает один бесплатный набор из 5 вопросов по позиции и грейду. `UNLIMITED_TELEGRAM_IDS` включает безлимит для указанных Telegram ID; свой ID можно посмотреть скрытой командой `/whoami`.
...
- `/update_resume 2` - обновить текст резюме номер 2.
- `/start` - прислать вакансию под активное резюме.

Если при добавлении резюме ввести уже существующее название, бот не создаст дубль и предложит обновить найденное резюме.
...
- После 5 вопросов бот покажет ограничение и заглушку платного пакета.

## Как работает банк вопросов

В `question_bank.json` лежит структурированный банк из 101 задачи и вопроса. Часть банка - базовые продуктовые/ML-кейсы для ДЗ, часть - реальные или близкие к реальным интервью-задачи, извлеченные и переформулированные из PDF-подборок:
...
- `case interview secrets - victor cheng (1).pdf` - высокоуровневые темы case interview; в банк добавлены оригинальные русские бизнес-кейсы по profitability, market sizing, pricing, market entry, growth, M&A и operations без дословного копирования книги.
- Дополнительные оригинальные блоки - SQL/data analyst, product sense/execution, system design, ML/data science, MLOps, backend/frontend, coding и behavioral STAR-вопросы по типам задач из популярных публичных interview-prep банков без копирования формулировок.
...
- `/update_resume 2` - обновить текст резюме по номеру.
